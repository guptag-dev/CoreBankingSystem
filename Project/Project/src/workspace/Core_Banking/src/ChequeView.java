...
/**
 * Servlet implementation class for Servlet: ChequeView
 *
 */
 public class ChequeView extends javax.servlet.http.HttpServlet implements javax.servlet.Servlet {
   static final long serialVersionUID = 1L;
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		else
		{
			...
			String EmpUid=per;
			String chquenum = request.getParameter("Chequeno");
			
			query = "select * from status where status.chqnum='"+chquenum+"'" ;
				rset=stmt.executeQuery(query);
			if(rset.next())	
			{
				String status=rset.getString(2);
				pw.println("<td valign=\"top\" width=580>Cheque Status: <B>"+status+"</B></td>");	
			}
			else
			{
			pw.println("<td valign=\"top\" width=580>Either the cheque has not been issued till yet or the cheque is pending</td>");
			}		
